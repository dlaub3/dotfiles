...
KITTY_WINDOW_ID=$( i3-msg -t get_tree | jq -r '.. | objects | select(.window) | select(.window_properties?.instance =="kitty").window')

for WINDOW_ID in $BTOP_WINDOW_ID $CHAT_GPT_WINDOW_ID $CLAUD_WINDOW_ID $KITTY_WINDOW_ID; do
  echo $WINDOW_ID
  if [ -n "$WINDOW_ID" ]; then
